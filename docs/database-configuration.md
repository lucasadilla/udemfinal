...
same MongoDB database. If `MONGODB_DB_NAME` is omitted, the application
automatically falls back to the database encoded in the URI or, if none is
present, defaults to `udemfinal`.
