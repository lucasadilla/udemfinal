...
function extractGridFsDescriptorFromUrl(url) {
  ...
  // Handle relative paths directly without needing a base URL
  if (url.startsWith('/api/podcasts/media/')) {
    const segments = url.split('/').filter(Boolean);
    const fileId = segments[segments.length - 1] || null;
    return { bucketName: 'podcastMedia', fileId };
  }
  if (url.startsWith('/api/user-images/')) {
    const segments = url.split('/').filter(Boolean);
    const fileId = segments[segments.length - 1] || null;
    return { bucketName: 'userImages', fileId };
  }

  // For absolute URLs, parse them properly
  try {
    // Use a generic base URL for parsing (not localhost)
