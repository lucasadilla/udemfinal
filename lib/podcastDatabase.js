...
function persistFallbackPodcasts(nextPodcasts) {
  fallbackCache = nextPodcasts;
  fallbackCacheLoaded = true;
  try {
    writeJsonFile(FALLBACK_FILE, nextPodcasts);
  } catch (error) {
    if (isReadOnlyFileSystemError(error)) {
      console.warn(
        'Système de fichiers en lecture seule détecté; les balados ajoutés seront conservés uniquement en mémoire.',
      );
      return;
    }
    throw error;
...
export async function getPodcasts() {
  try {
    ...
    const items = loadFallbackPodcasts();
    return items
...
export async function addPodcast(podcast) {
  const payload = {
    ...
    video: podcast.video,
    image: podcast.image,
    videoFileId: podcast.videoFileId || null,
    imageFileId: podcast.imageFileId || null,
    slug: podcast.slug,
    bio: podcast.bio || '',
    createdAt: podcast.createdAt || new Date().toISOString(),
    mediaSource: podcast.mediaSource || 'upload',
    imageSource: podcast.imageSource || 'upload',
  };
  ...
  try {
    ...
    const podcasts = [...loadFallbackPodcasts()];
    const fallbackPodcast = {
      ...payload,
      id: crypto.randomUUID(),
    };
    podcasts.push(fallbackPodcast);
    persistFallbackPodcasts(podcasts);
...
export async function deletePodcastById(id) {
  ...
  try {
    ...
    if (objectId) {
      ...
      if (result.value) {
        const normalized = normalizePodcast(result.value);
        return {
          ...normalized,
          videoFileId: result.value.videoFileId ?? normalized.videoFileId ?? null,
          imageFileId: result.value.imageFileId ?? normalized.imageFileId ?? null,
        };
      }
    }
  } catch (error) {
